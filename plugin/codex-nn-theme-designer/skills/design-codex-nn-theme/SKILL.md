...
5. 确认后生成一张 16:9 纯背景图，并选择现有 `layoutPreset`，设置 `appearance`、焦点、安全区、聊天页模式、配色和文案。背景不得包含按钮、输入框、完整 UI、文字、Logo 或水印。
...
- 主体放在一侧，另一侧保留低信息安全区；`taskMode` 缺省或设为 `auto` 时按图片比例自动选择聊天页效果，只有需要关闭聊天页背景时才显式设置 `off`。
